...
import { departments } from "../domain/Lead";
import { FetchHTTPClient } from "../infra/http/FetchHTTPClient";

dotenv.config();

export async function findOrganizationPotentialHiringManagers({
  company,
  job,
  lead,
}: {
  ...
  if (data.people && data.people.length > 0) {
    ...
    let jobLocations = job.location
      ?.split(",")
      .map((location) => location.replace(/[^a-zA-Z\s]/g, ""))
      .map((location) => location.trim())
      .filter(
        (location) => location !== "Remote" && location !== lead.location[0]
      );

    return {
      potentialHiringManagers:
        potentialHiringManagers?.sort((a, b) => {
          let aPoints = 0;
          let bPoints = 0;

          const aIsFromDepartment = departments.some((department) =>
            a.departments?.includes(department)
          );
          const bIsFromDepartment = departments.some((department) =>
            b.departments?.includes(department)
          );
          ...
          if (company.employeeCount < 50) {
            if (
              a.title?.includes("CEO") ||
              a.departments?.includes("founder")
            ) {
              aPoints += 0.2;
            }
            if (
              b.title?.includes("CEO") ||
              b.departments?.includes("founder")
            ) {
              bPoints += 0.2;
            }
          }
